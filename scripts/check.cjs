...
for(const [,attrs] of scripts){assert(!/\b(?:async|defer|type)\s*(?:=|$)/.test(attrs));const src=attrs.match(/src="([^"]+)"/)[1];assert(/^js\/[\w-]+\.js$/.test(src));new vm.Script(read(src),{filename:src});}
assert(html.includes('href="css/app.css"'));read('css/app.css');
