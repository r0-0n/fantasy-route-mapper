...
const ids=[...html.matchAll(/\bid="([^"]+)"/g)].map(x=>x[1]);assert.equal(ids.length,new Set(ids).size);for(const m of all.matchAll(/\$\(['"]#([\w-]+)['"]\)/g))assert(ids.includes(m[1]),m[1]);assert.equal(run('APP_VERSION'),'1.3.3');assert.equal(run('CURRENT_DATA_VERSION'),1);assert.equal(run('CURRENT_BACKUP_VERSION'),1);
(async()=>{
